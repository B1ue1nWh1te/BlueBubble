    ...
    <div class="container" id="comments">
        <div class="content">
            <div class="row align-items-center justify-content-center">
                <h3><?php $this->commentsNum(_t('暂无留言'), _t('仅有一条留言'), _t('%d 条留言')); ?></h3>
            </div>
            ...
            <div class="row align-items-center justify-content-center">
                <nav class="page-nav">
                    <?php $comments->pageNav('<i class="fa fa-angle-left" aria-hidden="true"></i>', '<i class="fa fa-angle-right" aria-hidden="true"></i>', 1, '...', array('wrapTag' => 'ul', 'wrapClass' => 'pagination', 'textTag' => 'a', 'currentClass' => 'active', 'prevClass' => '', 'nextClass' => '')); ?>
                </nav>
            </div>
            <?php endif; ?>
            <div class="comment-card">
                ...
                <div id="<?php $this->respondId(); ?>" class="comment-reply">
                    ...
                    <form method="post" action="<?php $this->commentUrl() ?>" id="comment-form" role="form"
                        class="container" style="overflow: auto; zoom: 1;">
                        <?php if ($this->user->hasLogin()) : ?>
                        <p><?php _e('已登录为'); ?><a
                                href="<?php $this->options->profileUrl(); ?>"><?php $this->user->screenName(); ?></a>。<a
                                href="<?php $this->options->logoutUrl(); ?>" title="Logout"><?php _e('注销？'); ?></a></p>
                        <?php else : ?>
                        <div class="row">
                            <div class="col-md-4">
                        ...
                        <?php endif; ?>
                        <p>
                            <textarea rows="8" cols="50" name="text" id="textarea" class="form-control" required
                                placeholder="(评论审核通过后才会展示)"><?php $this->remember('text'); ?></textarea>
                        </p>
                        <p>
                            <button type="submit" class="btn btn-outline-success" id="add-comment-button"
                                style="float: right;"><?php _e('提交留言'); ?></button>
                        </p>
                    </form>
                </div>
                <?php else : ?>
    ...
    getcomments = function(eles) {
        ...
        for (var p = 0; p < eles.length; p++) {
            ...
            for (var j = 0; j < lis.length; j++) {
                var h5 = $(lis[j]).children("div[id^='comment']").children(".comment-item").children(
                    ".comment-body").children(".comment-head").children("h5")
                var name
        ...
        $.ajax({
            ...
            success: function(html) {
                ...
                if (newdocument.title == "Error") {
                    ...
                    bindsubmit()
                    var authorName = $("#author").val() ? $("#author").val() : $(
                        "a[href$='profile.php']").text()
                    if (authorName) {
...
bindsubmit()
var rubbishScripts = new DOMParser().parseFromString(`<?php echo $this->pluginHandle()->header("", $this); ?>`,
    "text/html").getElementsByTagName("script")
var script
